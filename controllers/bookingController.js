...
export const getAllIncoming = async (req, res) => {
  ...
  try {
    ...
    const userBookings = await UserBookingMd.findOne({ userId: user._id }).populate('userId');
...
export const acceptBooking = async (req, res) => {
  try {
    const { id } = req.params;
    const userBookings = await UserBookingMd.findOne({ 'bookings._id': id });
